...
Current registry baseline after the next release: `@promptframe/contracts@0.1.7`, `@promptframe/component-kit@0.1.7`, `@promptframe/cli@0.1.19`, `create-promptframe-component@0.1.11`. `@promptframe/contracts@0.1.7` exposes the public authoring standard release, upload target policy, freshness decision schema, component reusability score schema, style intent contract, and `component.style.unknown_custom_style_prop` helper for detecting root-level private style props. `@promptframe/component-kit@0.1.7` sources its public standard stamp and style helper contract from `@promptframe/contracts`, and exposes `createPreviewCaseMatrix()` for bounded local preview aspect / props stress cases; `@promptframe/cli@0.1.19` includes `check` / `upgrade` lifecycle diagnostics, blocks stale PromptFrame authoring package floors before network upload for both folders and source zip archives, checks the platform standard source hash before sending package bytes, reports offline degraded freshness for `dev` / `check` when no platform endpoint is available, returns `localReusability` diagnostics from `check` and directory `upload`, reports unknown root-level private style props from `validate` / `check`, can forward explicit dev-header auth roles / permissions to guarded local platform endpoints, writes local preview reports from canonical and saved preview cases, and ignores package manager lockfiles during source safety scans and source package creation. `create-promptframe-component@0.1.11` scaffolds templates that use current public package ranges, include a local video preview shell with schema-derived props controls, 16:9 / 9:16 / 1:1 aspect presets, bounded auto preview cases, saved local preview case export/reporting for `.promptframe/local-previews/`, and support explicit `--name` / `--force` for legacy wrapper compatibility.
