...
Current npm registry baseline is `@promptframe/contracts@0.1.14`, `@promptframe/component-kit@0.1.11`, `@promptframe/cli@0.1.39`, and `create-promptframe-component@0.1.29`. `@promptframe/contracts@0.1.14` exposes the public authoring standard release, upload target policy, freshness decision schema, component reusability score schema, style intent contract, public dependency policy, public resource policy/schema, public security policy digest, and AST-aware security evaluator subpath with red-team browser capability gates for Image/Audio beacons, Worker/SharedWorker, `window.open`, `postMessage`, iframe/script creation, browser fingerprinting, storage/cookie access, and dangerous HTML. `@promptframe/component-kit@0.1.11` sources its public standard stamp and style/resource helper contracts from `@promptframe/contracts`, and exposes `createPreviewCaseMatrix()` plus `promptFramePublicResource()` for runtime resource lookup with local fallback. `@promptframe/cli@0.1.39` includes lifecycle diagnostics, package freshness checks, remote standard source hash checks, local preview reports, preview props/schema unknown-prop validation against the exported `propsSchema`, bearer login, browser login code flow, `whoami` / `logout`, endpoint discovery, project list/current diagnostics, secret-free `.promptframerc` project context init, Project component create/list, self-service CI token create/list/revoke, formal-endpoint dev-header rejection, single-component and workspace GitHub workflow setup, dynamic workspace matrix discovery from `promptframe-workspace.json`, `RUNNER_LABELS` and pnpm 10 workflow support, pnpm workspace root lockfile evidence without component symlinks, `promptframe-workspace.json` validation, workspace root `check .` / `upload .` auto-detection, source metadata headers for workspace uploads, source validation backed by the contracts AST security evaluator with `securityPolicyDigest` / `securityEvaluatorMode` JSON output, public resource diagnostics, sanitized lockfile evidence packaging for platform admission, case-sensitive native tag checks so Remotion `<Img>` / `<Video>` are not mistaken for raw browser tags, and `status --fail-on-build-failed` so CI can fail when platform admission fails after upload. `create-promptframe-component@0.1.29` scaffolds single-component projects and advanced `--workspace` monorepos with safe append/merge for existing workspace roots, recursive structured complex props controls, Advanced JSON fallback in local preview, current public contracts, pnpm 10 workspace roots, one-level `components/*` plus optional `packages/*` workspace constraints, dev tooling kept in devDependencies, root-managed CI workflows that gate failed platform builds, and CLI dependency floors.
