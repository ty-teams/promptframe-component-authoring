...
function sync(argv: string[]): void {
  const dir = resolve(firstPositionalArg(argv) ?? '.');
  if (hasFlag(argv, '--apply') && hasFlag(argv, '--dry-run')) {
  ...
  const nextFiles = [
    ...
    {
      path: DEV_PUBLIC_RESOURCES_MODULE_PATH,
      contents: renderDevPublicResourcesModule(publicResources.entries),
    },
  ];
  const files = nextFiles.map((file) => {
    const absolutePath = join(dir, file.path);
    const current = existsSync(absolutePath) ? readFileSync(absolutePath, 'utf8') : undefined;
    const changed = current !== file.contents;
    const action: SyncFileAction = changed ? (apply ? 'updated' : 'would_update') : 'unchanged';
    if (apply && changed) {
  ...
  const output = {
    ...
    apply,
    previewShell: {
      status: previewShellStatus,
      supported: previewShellStatus === 'thin_shell',
    },
    files,
    diagnostics: [
      ...(previewShellStatus === 'legacy_fat_shell'
        ? [diagnostic(
          'sync.preview_shell.legacy_fat_shell',
          'warning',
          'Legacy fat PreviewRoot shell detected; sync refreshes it to the supported thin PreviewRoot powered by @promptframe/component-kit.',
        )]
        : []),
      ...publicResources.diagnostics,
    ],
    diagnostic: diagnostic(
      apply ? 'sync.applied' : 'sync.dry_run',
      'info',
      apply
        ? 'PromptFrame generated authoring shell files were synchronized to the thin PreviewRoot contract.'
        : 'PromptFrame sync dry-run completed. Re-run with --apply to write the thin PreviewRoot contract.',
    ),
  };
  if (hasFlag(argv, '--json')) {
    printJson(output);
  ...
  console.log(apply ? `sync applied: ${dir}` : `sync dry-run: ${dir}`);
...
async function dev(argv: string[]): Promise<void> {
  ...
  const publicResources = evaluateDirectoryPublicResources(dir);
  assertPublicResourcesAccepted(publicResources);
  const devPublicResources = writeDevPublicResourceArtifacts(dir, publicResources);
...
function writeDevPublicResourceArtifacts(dir: string, publicResources: PublicResourceReport): {
  schemaVersion: typeof DEV_PUBLIC_RESOURCES_SCHEMA_VERSION;
  status: PublicResourceReportStatus;
  reportPath: typeof DEV_PUBLIC_RESOURCES_REPORT_PATH;
  generatedModulePath: typeof DEV_PUBLIC_RESOURCES_MODULE_PATH;
  entries: ComponentPublicResourceEntry[];
  total: number;
  totalBytes: number;
} {
  const payload = {
    schemaVersion: DEV_PUBLIC_RESOURCES_SCHEMA_VERSION,
    generatedAt: new Date().toISOString(),
    contractVersion: publicResources.contractVersion,
    policyVersion: publicResources.policyVersion,
    status: publicResources.status,
    total: publicResources.total,
    totalBytes: publicResources.totalBytes,
    entries: publicResources.entries,
  };
  mkdirSync(join(dir, '.promptframe'), { recursive: true });
  mkdirSync(join(dir, 'src'), { recursive: true });
  writeFileSync(join(dir, DEV_PUBLIC_RESOURCES_REPORT_PATH), `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
  writeFileSync(join(dir, DEV_PUBLIC_RESOURCES_MODULE_PATH), renderDevPublicResourcesModule(publicResources.entries), 'utf8');
  return {
    schemaVersion: DEV_PUBLIC_RESOURCES_SCHEMA_VERSION,
    status: publicResources.status,
    reportPath: DEV_PUBLIC_RESOURCES_REPORT_PATH,
    generatedModulePath: DEV_PUBLIC_RESOURCES_MODULE_PATH,
    entries: publicResources.entries,
    total: publicResources.total,
    totalBytes: publicResources.totalBytes,
  };
...
function renderDevPublicResourcesModule(entries: ComponentPublicResourceEntry[]): string {
  return [
    ...
    '};',
    '',
    `export const promptFrameDevPublicResources: readonly PromptFrameDevPublicResource[] = ${JSON.stringify(entries, null, 2)};`,
    '',
  ].join('\n');
}

function classifyPreviewShell(source: string | undefined): PreviewShellStatus {
  if (!source) return 'missing';
  if (/PromptFramePreviewApp/.test(source)) return 'thin_shell';
  if (/function\s+PreviewApp\b/.test(source) || /previewMessages/.test(source) || /renderLocalResourcePicker/.test(source)) {
    return 'legacy_fat_shell';
  }
  return 'custom_or_unknown';
}

function renderPromptFrameThinPreviewRoot(): string {
  return `import { Player } from '@remotion/player';
import {
...
Commands:
  ...
  sync <dir>                       Refresh PromptFrame-generated authoring shell files (--dry-run by default; use --apply)
