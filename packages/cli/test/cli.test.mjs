    ...
    for (const command of ['dev', 'check']) {
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.match(payload.diagnostic.message, /@promptframe\/cli@0\.1\.59/);
...
test('same-sourceHash recommended-only update stays non-blocking and is actionable in human output', async () => {
  ...
  try {
    ...
    assert.equal(jsonDev.freshness.localReleaseId, 'authoring-release-2026-07-10.3');
    ...
    assert.match(humanDev.stdout, /@promptframe\/cli@0\.1\.59/);
...
test('candidate promotion stage is non-blocking but never reports stable current', async () => {
  ...
  const server = await createServer(async (req, res) => {
    ...
    if (req.url === '/components/standard') {
      writeJson(res, {
        ...
        sourceHash: 'sha256:7ddd4fb8b37786a61c059af5ee41e808d1f3beb118efa1cdaa7dba825dd53e47',
        authoringStandardRelease: {
          releaseId: 'authoring-release-2026-07-10.3',
  ...
  try {
    ...
    assert.match(dev.freshness.diagnostic.message, /contracts=0\.1\.26/);
    assert.match(dev.freshness.diagnostic.message, /cli=0\.1\.58/);
  ...
  try {
    ...
    await writeFile(path.join(componentDir, 'package.json'), JSON.stringify({
      ...
      dependencies: {
        ...
        '@promptframe/contracts': '^0.1.26',
        '@promptframe/component-kit': '^0.1.20',
      },
      devDependencies: {
...
test('local standard, doctor, and validate expose stable JSON diagnostics', async () => {
  ...
  try {
    ...
    assert.equal(standard.authoringStandardRelease.releaseId, 'authoring-release-2026-07-10.3');
...
test('check and upgrade expose freshness and package floor diagnostics', async () => {
  ...
  try {
    ...
    assert.equal(appliedPackageJson.dependencies['@promptframe/contracts'], '^0.1.26');
    assert.equal(appliedPackageJson.dependencies['@promptframe/component-kit'], '^0.1.20');
    assert.equal(appliedPackageJson.devDependencies['@promptframe/cli'], '^0.1.58');
...
test('doctor and upgrade --check-latest expose stale scaffold template metadata', async () => {
  ...
  try {
    ...
    assert.equal(doctorDiagnostic.minimum, '0.1.48');
    ...
    assert.equal(sameVersionDiagnostic.current, '0.1.48');
