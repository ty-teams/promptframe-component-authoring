    ...
    for (const command of ['dev', 'check']) {
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.match(payload.diagnostic.message, /@promptframe\/cli@0\.1\.61/);
...
test('same-sourceHash recommended-only update stays non-blocking and is actionable in human output', async () => {
  ...
  try {
    ...
    assert.equal(jsonDev.freshness.localReleaseId, 'authoring-release-2026-07-10.5');
    ...
    assert.match(humanDev.stdout, /@promptframe\/cli@0\.1\.61/);
...
test('candidate promotion stage is non-blocking but never reports stable current', async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'promptframe-cli-candidate-promoting-'));
  const calls = [];
  const server = await createServer(async (req, res) => {
    calls.push(req.url);
    if (req.url === '/components/standard') {
      writeJson(res, {
        ...
        sourceHash: 'sha256:7ddd4fb8b37786a61c059af5ee41e808d1f3beb118efa1cdaa7dba825dd53e47',
        authoringStandardRelease: {
          releaseId: 'authoring-release-2026-07-10.5',
        ...
        releasePromotion: {
          stage: 'candidate',
          responsibility: 'platform_release_maintainer',
        },
      });
      return;
    }
    if (req.url === '/components/marketplace/upload') {
      writeJson(res, { success: true, buildId: 'candidate-upload', status: 'queued' });
      return;
    }
    writeJson(res, { success: false, error: `unexpected path: ${req.url}` }, 404);
  });
  try {
    ...
    assert.equal(dev.freshness.diagnostic.code, 'standard.release.promoting');
    assert.equal(dev.freshness.currentReleaseStage, 'candidate');
    assert.equal(dev.freshness.promotionResponsibility, 'platform_release_maintainer');
    assert.match(dev.freshness.diagnostic.message, /contracts=0\.1\.28/);
    assert.match(dev.freshness.diagnostic.message, /cli=0\.1\.60/);
    assert.match(dev.freshness.diagnostic.message, /does not claim stable\/current release completion/);
    ...
    assert.equal(upload.jobId, 'candidate-upload');
    assert.deepEqual(calls, [
      '/components/standard',
      '/components/standard',
      '/components/marketplace/upload',
  ...
  try {
    ...
    await writeFile(path.join(componentDir, 'package.json'), JSON.stringify({
      ...
      dependencies: {
        ...
        '@promptframe/contracts': '^0.1.28',
        '@promptframe/component-kit': '^0.1.22',
      },
      devDependencies: {
        '@promptframe/cli': '^0.1.60',
      },
    }, null, 2));
    await writeFile(path.join(componentDir, 'pnpm-lock.yaml'), 'lockfileVersion: "9.0"\n');
...
test('local standard, doctor, and validate expose stable JSON diagnostics', async () => {
  ...
  try {
    ...
    assert.equal(standard.authoringStandardRelease.releaseId, 'authoring-release-2026-07-10.5');
    ...
    assert.deepEqual(
      ...
      standard.authoringStandardRelease.recommendedAuthoringPackages,
    );
...
test('check and upgrade expose freshness and package floor diagnostics', async () => {
  ...
  try {
    ...
    assert.equal(appliedPackageJson.dependencies['@promptframe/contracts'], '^0.1.28');
    assert.equal(appliedPackageJson.dependencies['@promptframe/component-kit'], '^0.1.22');
    assert.equal(appliedPackageJson.devDependencies['@promptframe/cli'], '^0.1.60');
...
test('doctor and upgrade --check-latest expose stale scaffold template metadata', async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'promptframe-cli-scaffold-freshness-'));
  try {
    const componentDir = path.join(dir, 'component');
    const currentReactRemotionTemplateDigest = 'sha256:3e3456468d1153cb799ca0f510393a94138fdbc9ad7f9e1557aadc0d437a842c';
    await writeFixtureComponent(componentDir);
    ...
    assert.equal(doctorDiagnostic.minimum, '0.1.50');
    ...
    assert.equal(sameVersionDiagnostic.current, '0.1.50');
...
test('sync refreshes legacy fat preview shell without overwriting author component files', async () => {
  ...
  try {
    ...
    await writeFile(previewRootPath, [
      ...
      'const previewLocale = navigator.languages?.[0] ?? "en";',
      'function PreviewApp() {',
      '  useState({});',
      '  return previewLocale ? null : null;',
      '}',
    ].join('\n'));
    ...
    assert.doesNotMatch(previewRoot, /navigator\.languages|navigatorLanguages/);
...
test('sync still blocks security violations in author component source', async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'promptframe-cli-sync-component-security-'));
  try {
    const componentDir = path.join(dir, 'component');
    await writeFixtureComponent(componentDir);
    await writeFile(path.join(componentDir, 'src/PreviewRoot.tsx'), [
    ...
    await writeFile(path.join(componentDir, 'src/Component.tsx'), [
      ...
      "  return <AbsoluteFill>{locale}</AbsoluteFill>;",
      "}",
    ].join('\n'));
    ...
    await assert.rejects(
      ...
      (error) => {
        ...
        assert.match(output, /browser\.fingerprint/);
        assert.match(output, /src\/Component\.tsx/);
        assert.doesNotMatch(output, /src\/PreviewRoot\.tsx/);
        return true;
      },
    );
  } finally {
...
test('validate rejects wrappers with conditional returns or destructive side effects', async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'promptframe-cli-schema-wrapper-attacks-'));
  try {
    const componentDir = path.join(dir, 'component');
    await writeFixtureComponent(componentDir);
    const wrappers = [
