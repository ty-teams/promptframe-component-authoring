...
test('public templates use the current PromptFrame authoring package baseline', async () => {
  for (const templatePackagePath of [
    ...
    assert.equal(packageJson.dependencies?.['@promptframe/component-kit'], '^0.1.22', templatePackagePath);
    assert.equal(packageJson.dependencies?.['@promptframe/contracts'], '^0.1.28', templatePackagePath);
    ...
    assert.equal(packageJson.devDependencies?.['@promptframe/cli'], '^0.1.60', templatePackagePath);
...
test('public authoring docs document the current source baseline', async () => {
  for (const docPath of [
    ...
    assert.match(text, /@promptframe\/contracts@0.1.28/, docPath);
    assert.match(text, /@promptframe\/component-kit@0\.1\.22/, docPath);
    assert.match(text, /@promptframe\/cli@0.1.60/, docPath);
    assert.match(text, /create-promptframe-component@0.1.50/, docPath);
