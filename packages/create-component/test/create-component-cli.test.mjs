...
test('create CLI scaffolds an advanced workspace with one component', async () => {
  ...
  try {
    ...
    assert.match(stdout, /Next steps: pnpm install && pnpm run check/);
    ...
    assert.equal(rootPackage.devDependencies?.['@promptframe/cli'], '^0.1.59');
...
test('create CLI appends workspace components without overwriting root state', async () => {
  ...
  try {
    ...
    assert.equal(nextPackage.devDependencies['@promptframe/cli'], '^0.1.59');
