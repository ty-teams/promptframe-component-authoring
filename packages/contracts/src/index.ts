...
export const COMPONENT_STANDARD_SOURCE_HASH = 'sha256:7ddd4fb8b37786a61c059af5ee41e808d1f3beb118efa1cdaa7dba825dd53e47' as const;
...
export const AUTHORING_STANDARD_RELEASE_ID = 'authoring-release-2026-07-10.5' as const;
...
export const PROMPTFRAME_PUBLIC_SECURITY_POLICY = {
  ...
  forbiddenApis: [
    ...
    {
      id: 'browser.image_beacon',
      label: 'Image constructor network beacon',
      patterns: ['\\bnew\\s+(?:(?:window|globalThis)\\.)?Image\\s*\\(|\\b(?:window|globalThis)\\.Image\\s*\\('],
      astMatchers: {
        globals: ['Image'],
        memberPaths: ['window.Image', 'globalThis.Image'],
      },
      action: 'reject',
      category: 'network_exfiltration',
      severity: 'high',
      reason: 'Third-party components cannot create browser Image beacons that can send unmediated network requests.',
      recommendation: 'Use Remotion <Img> with platform-provided asset props or PromptFrame public resources; do not construct browser Image objects.',
      docsPath: '/docs/component-authoring/security#browser-image-beacon',
    },
    {
      id: 'browser.audio_beacon',
      label: 'Audio constructor network beacon',
      patterns: ['\\bnew\\s+(?:(?:window|globalThis)\\.)?Audio\\s*\\(|\\b(?:window|globalThis)\\.Audio\\s*\\('],
      astMatchers: {
        globals: ['Audio'],
        memberPaths: ['window.Audio', 'globalThis.Audio'],
      },
      action: 'reject',
      category: 'network_exfiltration',
      severity: 'high',
      reason: 'Third-party components cannot create browser Audio objects that can fetch remote media outside the platform asset boundary.',
      recommendation: 'Use platform-provided audio assets and Remotion audio primitives rather than browser Audio constructors.',
      docsPath: '/docs/component-authoring/security#browser-audio-beacon',
    },
    {
      id: 'browser.worker_context',
      label: 'Worker / SharedWorker execution context',
      patterns: ['\\bnew\\s+(?:Worker|SharedWorker)\\b|\\b(?:Worker|SharedWorker)\\s*\\('],
      astMatchers: {
        globals: ['Worker', 'SharedWorker'],
        memberPaths: ['window.Worker', 'window.SharedWorker', 'globalThis.Worker', 'globalThis.SharedWorker'],
      },
      action: 'reject',
      category: 'browser_capability',
      severity: 'high',
      reason: 'Third-party components cannot create background execution contexts outside the render sandbox.',
      recommendation: 'Keep component work synchronous, bounded, and frame-driven; move heavy preparation into platform-controlled build/admission steps.',
      docsPath: '/docs/component-authoring/security#browser-worker-context',
    },
    {
      id: 'browser.window_open',
      label: 'window.open new browsing context',
      patterns: ['(?:window|globalThis)\\.open\\s*\\(|\\bopen\\s*\\('],
      astMatchers: {
        globals: ['open'],
        memberPaths: ['window.open', 'globalThis.open'],
      },
      action: 'reject',
      category: 'browser_capability',
      severity: 'high',
      reason: 'Third-party components cannot open new windows or tabs from preview/render code.',
      recommendation: 'Remove window.open calls and express outbound links as platform-reviewed metadata outside component execution.',
      docsPath: '/docs/component-authoring/security#browser-window-open',
    },
    {
      id: 'browser.cross_context_message',
      label: 'Cross-context postMessage',
      patterns: ['(?:window|globalThis|parent|top|opener)\\.postMessage\\s*\\(|\\bpostMessage\\s*\\('],
      astMatchers: {
        globals: ['postMessage'],
        memberPaths: ['window.postMessage', 'globalThis.postMessage', 'parent.postMessage', 'top.postMessage', 'opener.postMessage'],
      },
      action: 'reject',
      category: 'browser_capability',
      severity: 'high',
      reason: 'Third-party components cannot send messages to parent, opener, top, or other browser contexts.',
      recommendation: 'Use explicit component props and platform-owned events instead of postMessage from component code.',
      docsPath: '/docs/component-authoring/security#browser-cross-context-message',
    },
    {
      id: 'browser.dynamic_script',
      label: 'Dynamic script element creation',
      patterns: ['document\\.createElement\\s*\\(\\s*[\'"`]script[\'"`]'],
      astMatchers: {
        documentCreateElementTags: ['script'],
      },
      action: 'reject',
      category: 'dynamic_code_execution',
      severity: 'high',
      reason: 'Third-party components cannot create script elements at preview or render time.',
      recommendation: 'Use static imports that pass the public authoring build and security checks.',
      docsPath: '/docs/component-authoring/security#browser-dynamic-script',
    },
    {
      id: 'browser.iframe_escape',
      label: 'iframe boundary escape',
      patterns: ['<iframe\\b|document\\.createElement\\s*\\(\\s*[\'"`]iframe[\'"`]'],
      astMatchers: {
        documentCreateElementTags: ['iframe'],
        jsxElements: ['iframe'],
      },
      action: 'reject',
      category: 'browser_capability',
      severity: 'high',
      reason: 'Third-party components cannot create nested iframe boundaries inside the platform preview/render sandbox.',
      recommendation: 'Render ordinary React/Remotion content and use platform-reviewed embeds outside component code.',
      docsPath: '/docs/component-authoring/security#browser-iframe-escape',
    },
    {
      id: 'browser.fingerprint',
      label: 'Browser fingerprinting surface',
      patterns: ['navigator\\.(?:userAgent|language|languages|hardwareConcurrency|deviceMemory)|window\\.(?:innerWidth|innerHeight)|screen\\.(?:width|height)'],
      astMatchers: {
        memberPaths: [
          'navigator.userAgent',
          'navigator.language',
      ...
      action: 'reject',
      category: 'privacy_fingerprinting',
      severity: 'high',
      reason: 'Third-party components cannot read browser fingerprinting or viewport surfaces for profiling or exfiltration.',
      recommendation: 'Use explicit props, Remotion video config, or platform-provided layout inputs instead of browser environment fingerprints.',
      docsPath: '/docs/component-authoring/security#browser-fingerprint',
    },
    {
      ...
      severity: 'high',
      reason: 'Marketplace components must not read cookies, localStorage, or sessionStorage from the user browser.',
      recommendation: 'Use platform-provided props for state and never read browser storage from component code.',
      docsPath: '/docs/component-authoring/security#storage-browser-storage',
    },
    {
  ...
  warningApis: [
    {
      ...
      docsPath: '/docs/component-authoring/security#dom-dangerous-html',
    },
    {
    ...
    },
    {
      ...
      recommendation: 'Use useVideoConfig().fps with secondsToFrames(seconds, fps), createDurationTimeline(), createRevealPhases(), createFillProgress(), or timeline.at() from @promptframe/component-kit/timing.',
      repairHint: 'Replace hardcoded frame counts with secondsToFrames(seconds, fps), createRevealPhases(), createFillProgress(), or createDurationTimeline()/timeline.at() from @promptframe/component-kit/timing.',
...
export const PROMPTFRAME_ACTIVE_AUTHORING_PACKAGE_COHORT: AuthoringPackageCohort = authoringPackageCohortSchema.parse({
  contracts: { name: '@promptframe/contracts', version: '0.1.28' },
  componentKit: { name: '@promptframe/component-kit', version: '0.1.22' },
  cli: { name: '@promptframe/cli', version: '0.1.60' },
  createComponent: { name: 'create-promptframe-component', version: '0.1.50' },
});
...
export const PROMPTFRAME_AUTHORING_STANDARD_RELEASE: AuthoringStandardRelease = authoringStandardReleaseSchema.parse({
  ...
  releaseDigest: 'sha256:dcea279e29d88d4aa344746edfb96a573edfa5a6726ea18596c70d2b4e34d529',
